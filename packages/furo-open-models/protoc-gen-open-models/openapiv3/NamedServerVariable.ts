// Code generated by furo protoc-gen-open-models. DO NOT EDIT.
// protoc-gen-open-models version: ????

import { FieldNode, Registry, STRING } from '@furo/open-models/dist/index';
import {
  ServerVariable as OpenapiV3ServerVariable,
  type IServerVariable as IOpenapiV3ServerVariable,
  type TServerVariable as TOpenapiV3ServerVariable,
} from './ServerVariable';

/**
 * @interface INamedServerVariable
 *  Automatically-generated message used to represent maps of ServerVariable as ordered (name,value) pairs.
 */
export interface INamedServerVariable {
  /**
   *  Map key
   */
  name?: string;
  /**
   *  Mapped value
   */
  value?: IOpenapiV3ServerVariable;
}

/**
 * @interface TNamedServerVariable
 *  Automatically-generated message used to represent maps of ServerVariable as ordered (name,value) pairs.
 */
export interface TNamedServerVariable {
  /**
   *  Map key
   */
  name?: string;
  /**
   *  Mapped value
   */
  value?: TOpenapiV3ServerVariable;
}

/**
 * NamedServerVariable
 *  Automatically-generated message used to represent maps of ServerVariable as ordered (name,value) pairs.
 */
export class NamedServerVariable extends FieldNode {
  //  Map key
  private _name: STRING;

  //  Mapped value
  private _value: OpenapiV3ServerVariable;

  public __defaultValues: INamedServerVariable;

  constructor(
    initData?: INamedServerVariable,
    parent?: FieldNode,
    parentAttributeName?: string,
  ) {
    super(undefined, parent, parentAttributeName);
    this.__meta.typeName = 'openapi.v3.NamedServerVariable';
    this.__meta.description =
      'NamedServerVariable Automatically-generated message used to represent maps of ServerVariable as ordered (name,value) pairs.';

    this.__meta.nodeFields = [
      {
        fieldName: 'name',
        protoName: 'name',
        FieldConstructor: STRING,
        constraints: {},
        description: 'Map key',
      },
      {
        fieldName: 'value',
        protoName: 'value',
        FieldConstructor: OpenapiV3ServerVariable,
        constraints: {},
        description: 'Mapped value',
      },
    ];

    // Initialize the fields
    //  Map key
    this._name = new STRING(undefined, this, 'name');

    //  Mapped value
    this._value = new OpenapiV3ServerVariable(undefined, this, 'value');

    // Set required fields
    [].forEach(fieldName => {
      (
        this[fieldName as keyof NamedServerVariable] as FieldNode
      ).__meta.required = true;
    });

    // Default values from openAPI annotations
    this.__defaultValues = {};

    // Initialize the fields with init data
    if (initData !== undefined) {
      this.__fromLiteral({ ...this.__defaultValues, ...initData });
    } else {
      this.__fromLiteral(this.__defaultValues);
    }

    // Set readonly fields after the init, so child nodes are readonly too
    [].forEach(fieldName => {
      (this[fieldName as keyof NamedServerVariable] as FieldNode).__readonly =
        true;
    });

    this.__meta.isPristine = true;
  }

  //  Map key
  public get name(): STRING {
    return this._name;
  }

  public set name(v: string) {
    this.__PrimitivesSetter(this._name, v);
  }

  //  Mapped value
  public get value(): OpenapiV3ServerVariable {
    return this._value;
  }

  public set value(v: IOpenapiV3ServerVariable) {
    this.__TypeSetter(this._value, v);
  }

  fromLiteral(data: INamedServerVariable) {
    super.__fromLiteral(data);
  }

  toLiteral(): INamedServerVariable {
    return super.__toLiteral();
  }
}

Registry.register('openapi.v3.NamedServerVariable', NamedServerVariable);
