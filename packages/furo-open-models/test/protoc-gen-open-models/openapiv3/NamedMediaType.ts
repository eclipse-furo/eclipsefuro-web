// Code generated by furo protoc-gen-open-models. DO NOT EDIT.
// protoc-gen-open-models version: ????

import { FieldNode, Registry, STRING } from '@furo/open-models/dist/index';
import {
  MediaType as OpenapiV3MediaType,
  type IMediaType as IOpenapiV3MediaType,
  type TMediaType as TOpenapiV3MediaType,
} from './MediaType';

/**
 * @interface INamedMediaType
 *  Automatically-generated message used to represent maps of MediaType as ordered (name,value) pairs.
 */
export interface INamedMediaType {
  /**
   *  Map key
   */
  name?: string;
  /**
   *  Mapped value
   */
  value?: IOpenapiV3MediaType;
}

/**
 * @interface TNamedMediaType
 *  Automatically-generated message used to represent maps of MediaType as ordered (name,value) pairs.
 */
export interface TNamedMediaType {
  /**
   *  Map key
   */
  name?: string;
  /**
   *  Mapped value
   */
  value?: TOpenapiV3MediaType;
}

/**
 * NamedMediaType
 *  Automatically-generated message used to represent maps of MediaType as ordered (name,value) pairs.
 */
export class NamedMediaType extends FieldNode {
  //  Map key
  private _name: STRING;

  //  Mapped value
  private _value: OpenapiV3MediaType;

  public __defaultValues: INamedMediaType;

  constructor(
    initData?: INamedMediaType,
    parent?: FieldNode,
    parentAttributeName?: string,
  ) {
    super(undefined, parent, parentAttributeName);
    this.__meta.typeName = 'openapi.v3.NamedMediaType';
    this.__meta.description =
      'NamedMediaType Automatically-generated message used to represent maps of MediaType as ordered (name,value) pairs.';

    this.__meta.nodeFields = [
      {
        fieldName: 'name',
        protoName: 'name',
        FieldConstructor: STRING,
        constraints: {},
        description: 'Map key',
      },
      {
        fieldName: 'value',
        protoName: 'value',
        FieldConstructor: OpenapiV3MediaType,
        constraints: {},
        description: 'Mapped value',
      },
    ];

    // Initialize the fields
    //  Map key
    this._name = new STRING(undefined, this, 'name');

    //  Mapped value
    this._value = new OpenapiV3MediaType(undefined, this, 'value');

    // Set required fields
    [].forEach(fieldName => {
      (this[fieldName as keyof NamedMediaType] as FieldNode).__meta.required =
        true;
    });

    // Default values from openAPI annotations
    this.__defaultValues = {};

    // Initialize the fields with init data
    if (initData !== undefined) {
      this.__fromLiteral({ ...this.__defaultValues, ...initData });
    } else {
      this.__fromLiteral(this.__defaultValues);
    }

    // Set readonly fields after the init, so child nodes are readonly too
    [].forEach(fieldName => {
      (this[fieldName as keyof NamedMediaType] as FieldNode).__readonly = true;
    });

    this.__meta.isPristine = true;
  }

  //  Map key
  public get name(): STRING {
    return this._name;
  }

  public set name(v: string) {
    this.__PrimitivesSetter(this._name, v);
  }

  //  Mapped value
  public get value(): OpenapiV3MediaType {
    return this._value;
  }

  public set value(v: IOpenapiV3MediaType) {
    this.__TypeSetter(this._value, v);
  }

  fromLiteral(data: INamedMediaType) {
    super.__fromLiteral(data);
  }

  toLiteral(): INamedMediaType {
    return super.__toLiteral();
  }
}

Registry.register('openapi.v3.NamedMediaType', NamedMediaType);
