...
Validators.set("furo.type.DecimalRange", (node: DecimalRange) => {
  if (parseFloat(node.start.value.toString()) > parseFloat(node.end.value.toString())) {
    node.__setValueState(ValueState.Negative, ["decimal.start.must.be.smaller.then.end"]);
  } else {
...
Validators.set("furo.type.Defaults", (node: Defaults) => {
  if (node.id.value === "----") {
    node.id.__setValueState(ValueState.Negative, ["defaults.id.can.not.be.4", "----"]);
  } else {
    node.__setValueState(ValueState.None, [""]);
  }
});
...
Validators.set("furo.type.Decimal", (node: Decimal) => {
  if (!/^[+-]?(\d*\.)?\d+$/.exec(node.value.value)) {
    // maybe ^[+-]?(\d*\.)?\d*$   ???
    node.__setValueState(ValueState.Negative, ["decimal.invalid.number", node.value.value]);
  } else {
      ...
      for (const [constraint, value] of Object.entries(fieldConstraints)) {
        if (constraint === "minimum") {
          if (parseFloat(node.value.value) < value) {
            node.__setValueState(ValueState.Negative, ["constraint.violation.minimum", String(value), node.value.toString()]);
            return;
          }
        }
      }
...
Validators.set("furo.cube.CubeDefinition", (cube: CubeDefinition) => {
  ...
  if (cube.length.value >= 100 && cube.length.value <= 1000) {
    ...
    if (cube.length.value < avg * 0.9) {
      ...
      lengthState.message = "Please consider to make this value bigger, to have a perfect cube";
    }
    if (cube.length.value < avg * 0.8) {
      lengthState.state = ValueState.Critical;
      lengthState.message = "A perfect cube should have the same length, breadth and height.\n It is strongly recommended to increase this value";
    }
    ...
    if (cube.length.value === cube.breadth.value && cube.length.value === cube.height.value) {
      lengthState.state = ValueState.Positive;
    }
    if (cube.length.value > avg * 1.1) {
      lengthState.state = ValueState.Information;
      lengthState.message = "Please consider to make this value smaller, to have a perfect cube";
    }
  ...
  if (cube.breadth.value >= 100 && cube.breadth.value <= 1000) {
    ...
    if (cube.breadth.value < avg * 0.9) {
      ...
      breadthState.message = "Please consider to make this value bigger, to have a perfect cube";
    }

    if (cube.breadth.value < avg * 0.8) {
      breadthState.state = ValueState.Critical;
      breadthState.message = "A perfect cube should have the same breadth, breadth and height.\n It is strongly recommended to increase this value";
    }
    ...
    if (cube.breadth.value > avg * 1.1) {
      breadthState.state = ValueState.Information;
      breadthState.message = "Please consider to make this value smaller, to have a perfect cube";
    }
  ...
  if (cube.height.value >= 100 && cube.height.value <= 1000) {
    ...
    if (cube.height.value < avg * 0.9) {
      ...
      heightState.message = "Please consider to make this value bigger, to have a perfect cube";
    }

    if (cube.height.value < avg * 0.8) {
      heightState.state = ValueState.Critical;
      heightState.message = "A perfect cube should have the same height, height and height.\n It is strongly recommended to increase this value";
    }
    ...
    if (cube.height.value > avg * 1.1) {
      heightState.state = ValueState.Information;
      heightState.message = "Please consider to make this value smaller, to have a perfect cube";
    }
    if (cube.height.value > avg * 1.2) {
      heightState.state = ValueState.Critical;
      heightState.message = "A perfect cube should have the same height, breadth and height.\n It is strongly recommended to decrease this value";
    }
