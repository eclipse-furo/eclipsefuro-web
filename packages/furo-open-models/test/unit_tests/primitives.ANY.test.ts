...
describe("primitives ANY type", () => {
  ...
  it("should assign a IAny to a ANY field", async () => {
    ...
    expect(id.any.__stringify()).to.eql('{"value":"1234","@type":"furo.type.Decimal"}');
  });
  ...
  it("should use ANY types type as root for labels", async () => {
    ...
    expect(id.bookingCenter.__label).to.equal("FURO_TYPE_IDENTIFIER_BOOKING_CENTER_LABEL");
    expect((id.any.value as Identifier).bookingCenter.__label).to.equal("FURO_TYPE_IDENTIFIER_BOOKING_CENTER_LABEL");
  });

  it("should resolve the full path", async () => {
    const id = new Identifier(initData[0]);

    expect((id.any.value as Identifier).bookingCenter.__fieldPath).to.equal("any.value.booking_center");
    expect((id.any.value as Identifier).bookingCenter).to.equal(id.__getFieldNodeByPath("any.value.booking_center"));
  });
