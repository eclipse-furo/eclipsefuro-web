...
describe("primitives INT64 type", () => {
  it("must check constraints", async () => {
    ...
    expect(numeric.primitiveInt64.__meta.stateMessage).to.equal("constraint.violation.exclusive_maximum 1000 3442");
    numeric.primitiveInt64Excl = 1990n;
    expect(numeric.primitiveInt64Excl.__meta.stateMessage).to.equal("constraint.violation.maximum 1000 1990");
    ...
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt64.__meta.stateMessage).to.equal("constraint.violation.multiple_of 5 996");

    numeric.primitiveInt64 = -5n;
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt64.__meta.stateMessage).to.equal("constraint.violation.exclusive_minimum 5 -5");

    numeric.primitiveInt64Excl = -5n;
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt64Excl.__meta.stateMessage).to.equal("constraint.violation.minimum 5 -5");
    ...
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt64.__meta.stateMessage).to.equal("constraint.violation.exclusive_maximum 1000 1000");
  ...
  it("must reset the value state after __fromProtoNameJson", async () => {
    ...
    expect(numeric.__isValid).to.be.true;
    expect(numeric.primitiveInt64.__meta.valueState, "state is reset").to.equal("None");
    expect(numeric.primitiveInt64.__meta.stateMessage, "state is reset").to.equal("");
  });
  ...
  it("must accept a number[] on repeated INT64", async () => {
    ...
    expect(numeric.repeatedPrimitiveInt64.__toLiteral()).to.eql(["3", "4", "5", "6", "7"]);
    expect(numeric.repeatedPrimitiveInt64.length).to.eql(5);
  });

  it("must return an Array with values even on 0 vals", async () => {
    const numeric = new Numeric64(initData[2]);
    expect(numeric.repeatedPrimitiveInt64.length, "literal array with 0 vals").to.eql(4);
    expect(numeric.repeatedPrimitiveInt64.__isEmpty, "__isEmpty").to.equal(false);
    expect(numeric.repeatedPrimitiveInt64.__toLiteral(), "literal array with 0 vals").to.eql(["0", "0", "0", "0"]);
    expect(numeric.__toJson().repeated_primitive_int64).to.eql(["0", "0", "0", "0"]);
  });
