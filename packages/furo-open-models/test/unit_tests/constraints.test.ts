...
describe("Field Constraints", () => {
  ...
  it("should mark required empty strings as error", async () => {
    ...
    expect(c.id.__meta.stateMessage).to.equal("constraint.violation.min_length 3 ");
  ...
  it("should mark max length strings as error", async () => {
    ...
    expect(c.__isValid).to.be.false;
    expect(c.id.__meta.valueState).to.equal("Negative");
    expect(c.id.__meta.stateMessage).to.equal("constraint.violation.max_length 10 1234567890-");
  ...
  it("should check for pattern /^-*$/ ", async () => {
    ...
    expect(c.__isValid).to.be.false;
    expect(c.id.__meta.valueState).to.equal("Negative");
    expect(c.id.__meta.stateMessage).to.equal("constraint.violation.pattern ^-*$ ---3-");
  });
});
