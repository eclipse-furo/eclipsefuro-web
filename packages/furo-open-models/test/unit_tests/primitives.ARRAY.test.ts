...
const initData: IIdentifier[] = [
  ...
  },
  {
    ...
    repeatedDecimal: [{ value: "1234" }, { value: "12324" }, { value: "2.2" }, { value: "3.3" }],
    fatString: {
      value: "123",
      labels: { aaa: true, xxxx: false },
      attributes: { key: "BBBB", value: "stand." },
...
describe("primitives ARRAY type", () => {
  it("should accept literal on push", async () => {
    ...
    expect(id.stringArray.__toLiteral()).to.eql(["A", "B", "C", "stringliteral"]); // a,b,c is from the default
  });
  ...
  it("must return baseName without array index", async () => {
    ...
    expect(id.repeatedDecimal.value[1].value.__label).to.eql("FURO_TYPE_IDENTIFIER_REPEATED_DECIMAL_VALUE_LABEL");
  });
  ...
  it('should have the regular Array method "__stringify"', async () => {
    const id = new Identifier(initData[1]);
    expect(id.repeatedDecimal.__stringify()).to.eql('[{"value":"1234"},{"value":"12324"},{"value":"2.2"},{"value":"3.3"}]');
