...
import { StrictCubeService } from '../protoc-gen-open-models/furo/cube/CubeService';
...
describe('StrictFetcher', () => {
  ...
  it('should use proto names from field descriptors for query params when UseProtoNamesForQueryParams=true', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((request: Request) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({ entity: { display_name: 'Test' } }), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      }));
    }));

    const service = new StrictCubeService();
  ...
  it('should use camelCase keys for query params when UseProtoNamesForQueryParams=false', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((request: Request) => {
      capturedUrl = request.url;
      return Promise.resolve(new Response(JSON.stringify({ entity: { displayName: 'Test' } }), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      }));
    }));
  ...
  it('should use FieldNode serialization for body with bodyField="*" and UseProtoNames=true', async () => {
    ...
    const fetcher = new StrictFetcher(
      API_OPTIONS, 'POST', '/v1/test',
      CubeServiceGetRequest, CubeServiceGetResponse, '*',
    );
