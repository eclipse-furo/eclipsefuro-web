...
describe("primitives INT32 type", () => {
  it("must check constraints", async () => {
    ...
    expect(numeric.primitiveInt32.__meta.stateMessage).to.equal("constraint.violation.exclusive_maximum 1000 3442");
    ...
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt32.__meta.stateMessage).to.equal("constraint.violation.multiple_of 5 996");

    numeric.primitiveInt32 = -5;
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt32.__meta.stateMessage).to.equal("constraint.violation.exclusive_minimum 5 -5");
    ...
    expect(numeric.__isValid).to.be.false;
    expect(numeric.primitiveInt32.__meta.stateMessage).to.equal("constraint.violation.exclusive_maximum 1000 1000");
  ...
  it("must reset the value state after __fromProtoNameJson", async () => {
    ...
    expect(numeric.__isValid).to.be.true;
    expect(numeric.primitiveInt32.__meta.valueState, "state is reset").to.equal("None");
    expect(numeric.primitiveInt32.__meta.stateMessage, "state is reset").to.equal("");
  });
  ...
  it("must accept a number[] on repeated INT32", async () => {
    ...
    expect(numeric.repeatedPrimitiveInt32.__toLiteral()).to.eql([3, 4, 5, 6, 7]);
    expect(numeric.repeatedPrimitiveInt32.length).to.eql(5);
  });

  it("must return an Array with values even on 0 vals", async () => {
    const numeric = new Numeric(initData[2]);
    expect(numeric.repeatedPrimitiveInt32.length, "literal array with 0 vals").to.eql(4);
    expect(numeric.repeatedPrimitiveInt32.__isEmpty, "__isEmpty").to.equal(false);
    expect(numeric.repeatedPrimitiveInt32.__toLiteral(), "literal array with 0 vals").to.eql([0, 0, 0, 0]);
