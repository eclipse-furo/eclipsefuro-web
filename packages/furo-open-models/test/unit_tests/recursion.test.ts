...
describe("Recursion", () => {
  ...
  it("should create a initial tree", async () => {
    ...
    expect(tree.recursion.value!.recursion.value!.displayName.toString()).to.eql("C");
  ...
  it("should return a correct path", async () => {
    const tree = new Tree(initData[0]);
    expect(tree.recursion.value!.recursion.value!.displayName.toString()).equal("C");
    expect(tree.recursion.value!.recursion.value!.recursion.value!.displayName.toString()).equal("D");
    expect(tree.recursion.value!.recursion.value!.recursion.value!.displayName.__label).equal("FURO_TYPE_TREE_DISPLAY_NAME_LABEL");
    expect(tree.recursion.value!.recursion.value!.recursion.__label).equal("FURO_TYPE_TREE_RECURSION_LABEL");
    expect(tree.displayName.__fieldPath).equal("display_name");
    expect(tree.recursion.value!.recursion.value!.recursion.value!.displayName.__fieldPath).equal("recursion.recursion.recursion.display_name");
    expect(tree.recursion.value!.recursion.value!.recursion.value!.displayName.__label).equal("FURO_TYPE_TREE_DISPLAY_NAME_LABEL");
  });
});
